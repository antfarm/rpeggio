...
class Cells1DSound < Cells1D
  ...
  def play_rule_expressions
    ...
    @cells.each_index do |i|
      ...
      if play_rule? i
        #note = @synth.random_note @channel_id
        note = i - @play_range[:start]
        @synth.play_note(@channel_id, note, rand(30)+30)
      end

      # velocity = 64# @env.params(:midi, 1, 6) + rand(@env.params(:midi, 1, 7))
      # if play_rule? i
      #   #@synth.play_note(@channel_id, i - @play_range[:start], rand(30)+30)
      #
      #   note = i - @play_range[:start]
      #   duration = 10#@env.params(:midi, 1, 8);
      #
      ...
      #   @synth2.note_on(0, note, [[velocity, 127].min, 0].max)
      # end
    end
  end
